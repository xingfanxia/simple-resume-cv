...
\begin{Detail}
\SubBulletItem
Contributed to on-device user profiling framework which consists of several models to predict users' overall interest and preferences.
\SubBulletItem
Built listener services to collect feedback and loggings to better track engagement of suggestions and prediction provided.
\SubBulletItem
Improved the current location model using more diverse data sources with better filtering and data sanitation.
\SubBulletItem
Overhauled the topic prediction model with more up to date data source and sanitized features.
\SubBulletItem
Developed an automatic machine learning pipeline which retrains models with data source updates, launches experiments A/B testing the models, and deploys if a specified improvement is observed.
\SubBulletItem
Working on action prediction features which predicts what a user intends to do at a particular time and location based on various signals and models.
\end{Detail}
...
\BulletItem
Programming Languages: Proficiency in Golang, Python, Java, Javascript, Scala, C, Objective-C

\Gap
\BulletItem
Frameworks: VueJS, Bulma, LAMP, GraphQL, NodeJS, React, Django, ELK, Splunk, Apache Spark, Apache Kafka
...
\Gap
\BulletItem
Machine Learning \& Deep Learning: Scikit Learn, OpenGL, Theano, Matplotlib, Scipy, Numpy, Pandas, Keras, Jupyter, Private Federated Learning
